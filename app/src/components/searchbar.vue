<template>
  <form class="d-flex" @submit.prevent="beforeSearch">
    ...
    <select class="form-select w-50" aria-label="Default select example" ref="select" v-model="form.selected" v-if="this.$route.name.toLowerCase() === 'movies'">
...
export default {
  ...
  computed: {
    ActiveSelect() {
      return this.$route.name.toLowerCase() === 'movies'
    }
  },
  methods: {
    ...mapActions(['search']),
    async beforeSearch(){
      ...
      } else delete this.form.selected
